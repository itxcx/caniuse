...
Page({
  onLoad: function (res) {
    ...
    // console.log("shareKeyURL:" + app.option.query.shareKeyUrl)

    // console.log("path:" + app.userInfo)
    // console.log("shareKeyUrl:" + app.shareUrlId)
  ...
  bindconfirm: function() {
    ...
    this.setData({
      ...
      for(let p in app.post) {
        if(app.post[p].title.toLowerCase().match(this.data.inputValue.toLowerCase()) || app.post[p].keywords.toLowerCase().match(this.data.inputValue.toLowerCase())) {
          ...
          for(let a5 in app.post[p].stats.opera){
            if(app.post[p].stats.opera[a5].match(/n|不支持/ig)){
              app.post[p].stats.opera[a5] = "版本：" + a5 + " 及以下【不支持】"
              css3_s[15] = a5;
            }else if(app.post[p].stats.opera[a5].match(/a|p|部分支持/ig)){
              app.post[p].stats.opera[a5] = "版本：" + a5 + " 及以下【部分支持，可能需要浏览器前缀】"
              css3_s[16] = a5;
            }else if(app.post[p].stats.opera[a5].match(/y|已支持/ig)){
              app.post[p].stats.opera[a5] = "版本：" + a5 + " 【已支持】"
              css3_s[17] = a5;
            }
          ...
          for(let a6 in app.post[p].stats.ios_saf){
            if(app.post[p].stats.ios_saf[a6].match(/n|不支持/ig)){
              app.post[p].stats.ios_saf[a6] = "版本：" + a6 + " 及以下【不支持】"
              css3_s[18] = a6;
              css3_s[18] = Number.parseFloat(a6.replace(/(\d.*)[-]/ig,0))
            }else if(app.post[p].stats.ios_saf[a6].match(/a|p|部分支持/ig)){
              app.post[p].stats.ios_saf[a6] = "版本：" + a6 + " 及以下【部分支持，可能需要浏览器前缀】"
              css3_s[19] = Number.parseFloat(a6.replace(/[-](\d.*)/ig,".0"));
            }else if(app.post[p].stats.ios_saf[a6].match(/y|已支持/ig)){
              app.post[p].stats.ios_saf[a6] = "版本：" + a6 + " 【已支持】"
          ...
          for(let a7 in app.post[p].stats.op_mini){
            if(a7.match(/all/ig)){
              if(app.post[p].stats.op_mini[a7].match(/n|不支持/ig)){
                app.post[p].stats.op_mini[a7] = "版本：" + a7 + " 及以下【不支持】"
                css3_sR[21] = css3_s[21] = a7;
              }else if(app.post[p].stats.op_mini[a7].match(/a|p|部分支持/ig)){
                app.post[p].stats.op_mini[a7] = "版本：" + a7 + " 及以下【部分支持，可能需要浏览器前缀】"
            ...
            if(temp_y7 <= Number.parseFloat(css3_s[23]) && temp_y7 != 0) {
              css3_s[23] = temp_y7;
              console.log(app.post[p].stats.op_mini)
            }else{
              temp_y7 = css3_s[23];
            }
          ...
          for(let a8 in app.post[p].stats.android){
            if(app.post[p].stats.android[a8].match(/n|不支持/ig)){
              app.post[p].stats.android[a8] = "版本：" + a8 + " 及以下【不支持】"
              css3_s[24] = a8;
              css3_s[24] = Number.parseFloat(a8.replace(/(\d.*)[-]/ig,0))
            }else if(app.post[p].stats.android[a8].match(/a|p|部分支持/ig)){
              app.post[p].stats.android[a8] = "版本：" + a8 + " 及以下【部分支持，可能需要浏览器前缀】"
              css3_s[25] = Number.parseFloat(a8.replace(/[-](\d.*)/ig,".0"));
            }else if(app.post[p].stats.android[a8].match(/y|已支持/ig)){
              app.post[p].stats.android[a8] = "版本：" + a8 + " 【已支持】"
          ...
          for(let a9 in app.post[p].stats.bb){
            if(app.post[p].stats.bb[a9].match(/n|不支持/ig)){
              app.post[p].stats.bb[a9] = "版本：" + a9 + " 及以下【不支持】"
              css3_s[27] = a9;
              css3_s[27] = Number.parseFloat(a9.replace(/(\d.*)[-]/ig,0))
            }else if(app.post[p].stats.bb[a9].match(/a|p|部分支持/ig)){
              app.post[p].stats.bb[a9] = "版本：" + a9 + " 及以下【部分支持，可能需要浏览器前缀】"
              css3_s[28] = Number.parseFloat(a9.replace(/[-](\d.*)/ig,".0"));
            }else if(app.post[p].stats.bb[a9].match(/y|已支持/ig)){
              app.post[p].stats.bb[a9] = "版本：" + a9 + " 【已支持】"
          ...
          for(let a10 in app.post[p].stats.op_mob){
            if(app.post[p].stats.op_mob[a10].match(/n|不支持/ig)){
              app.post[p].stats.op_mob[a10] = "版本：" + a10 + " 及以下【不支持】"
              css3_s[30] = a10;
              css3_s[30] = Number.parseFloat(a10.replace(/(\d.*)[-]/ig,0))
            }else if(app.post[p].stats.op_mob[a10].match(/a|p|部分支持/ig)){
              app.post[p].stats.op_mob[a10] = "版本：" + a10 + " 及以下【部分支持，可能需要浏览器前缀】"
              css3_s[31] = Number.parseFloat(a10.replace(/[-](\d.*)/ig,".0"));
            }else if(app.post[p].stats.op_mob[a10].match(/y|已支持/ig)){
              app.post[p].stats.op_mob[a10] = "版本：" + a10 + " 【已支持】"
          ...
          for(let a11 in app.post[p].stats.and_chr){
            if(app.post[p].stats.and_chr[a11].match(/n|不支持/ig)){
              app.post[p].stats.and_chr[a11] = "版本：" + a11 + " 及以下【不支持】"
              css3_s[33] = a11;
              css3_s[33] = Number.parseFloat(a11.replace(/(\d.*)[-]/ig,0))
            }else if(app.post[p].stats.and_chr[a11].match(/a|p|部分支持/ig)){
              app.post[p].stats.and_chr[a11] = "版本：" + a11 + " 及以下【部分支持，可能需要浏览器前缀】"
              css3_s[34] = Number.parseFloat(a11.replace(/[-](\d.*)/ig,".0"));
            }else if(app.post[p].stats.and_chr[a11].match(/y|已支持/ig)){
              app.post[p].stats.and_chr[a11] = "版本：" + a11 + " 【已支持】"
          ...
          for(let a12 in app.post[p].stats.and_ff){
            if(app.post[p].stats.and_ff[a12].match(/n|不支持/ig)){
              app.post[p].stats.and_ff[a12] = "版本：" + a12 + " 及以下【不支持】"
              css3_s[36] = a12;
              css3_s[36] = Number.parseFloat(a12.replace(/(\d.*)[-]/ig,0))
            }else if(app.post[p].stats.and_ff[a12].match(/a|p|部分支持/ig)){
              app.post[p].stats.and_ff[a12] = "版本：" + a12 + " 及以下【部分支持，可能需要浏览器前缀】"
              css3_s[37] = Number.parseFloat(a12.replace(/[-](\d.*)/ig,".0"));
            }else if(app.post[p].stats.and_ff[a12].match(/y|已支持/ig)){
              app.post[p].stats.and_ff[a12] = "版本：" + a12 + " 【已支持】"
          ...
          for(let a13 in app.post[p].stats.ie_mob){
            if(app.post[p].stats.ie_mob[a13].match(/n|不支持/ig)){
              app.post[p].stats.ie_mob[a13] = "版本：" + a13 + " 及以下【不支持】"
              css3_s[39] = a13;
              css3_s[39] = Number.parseFloat(a13.replace(/(\d.*)[-]/ig,0))
            }else if(app.post[p].stats.ie_mob[a13].match(/a|p|部分支持/ig)){
              app.post[p].stats.ie_mob[a13] = "版本：" + a13 + " 及以下【部分支持，可能需要浏览器前缀】"
              css3_s[40] = Number.parseFloat(a13.replace(/[-](\d.*)/ig,".0"));
            }else if(app.post[p].stats.ie_mob[a13].match(/y|已支持/ig)){
              app.post[p].stats.ie_mob[a13] = "版本：" + a13 + " 【已支持】"
          ...
          for(let a14 in app.post[p].stats.and_uc){
            if(app.post[p].stats.and_uc[a14].match(/n|不支持/ig)){
              app.post[p].stats.and_uc[a14] = "版本：" + a14 + " 及以下【不支持】"
              css3_s[42] = a14;
              css3_s[42] = Number.parseFloat(a14.replace(/(\d.*)[-]/ig,0))
            }else if(app.post[p].stats.and_uc[a14].match(/a|p|部分支持/ig)){
              app.post[p].stats.and_uc[a14] = "版本：" + a14 + " 及以下【部分支持，可能需要浏览器前缀】"
              css3_s[43] = Number.parseFloat(a14.replace(/[-](\d.*)/ig,".0"));
            }else if(app.post[p].stats.and_uc[a14].match(/y|已支持/ig)){
              app.post[p].stats.and_uc[a14] = "版本：" + a14 + " 【已支持】"
          ...
          for(let a15 in app.post[p].stats.samsung){
            if(app.post[p].stats.samsung[a15].match(/n|不支持/ig)){
              app.post[p].stats.samsung[a15] = "版本：" + a15 + " 及以下【不支持】"
              css3_s[45] = a15;
              css3_s[45] = Number.parseFloat(a15.replace(/(\d.*)[-]/ig,0))
            }else if(app.post[p].stats.samsung[a15].match(/a|p|部分支持/ig)){
              app.post[p].stats.samsung[a15] = "版本：" + a15 + " 及以下【部分支持，可能需要浏览器前缀】"
              css3_s[46] = Number.parseFloat(a15.replace(/[-](\d.*)/ig,".0"));
            }else if(app.post[p].stats.samsung[a15].match(/y|已支持/ig)){
              app.post[p].stats.samsung[a15] = "版本：" + a15 + " 【已支持】"
          ...
          for(let a16 in app.post[p].stats.and_qq){
            if(app.post[p].stats.and_qq[a16].match(/n|不支持/ig)){
              app.post[p].stats.and_qq[a16] = "版本：" + a16 + " 及以下【不支持】"
              css3_s[48] = a16;
              css3_s[48] = Number.parseFloat(a16.replace(/(\d.*)[-]/ig,0))
            }else if(app.post[p].stats.and_qq[a16].match(/a|p|部分支持/ig)){
              app.post[p].stats.and_qq[a16] = "版本：" + a16 + " 及以下【部分支持，可能需要浏览器前缀】"
              css3_s[49] = Number.parseFloat(a16.replace(/[-](\d.*)/ig,".0"));
            }else if(app.post[p].stats.and_qq[a16].match(/y|已支持/ig)){
              app.post[p].stats.and_qq[a16] = "版本：" + a16 + " 【已支持】"
          ...
          for(let a17 in app.post[p].stats.baidu){
            if(app.post[p].stats.baidu[a17].match(/n|不支持/ig)){
              app.post[p].stats.baidu[a17] = "版本：" + a17 + " 及以下【不支持】"
              css3_s[51] = a17;
              css3_s[51] = Number.parseFloat(a17.replace(/(\d.*)[-]/ig,0))
            }else if(app.post[p].stats.baidu[a17].match(/a|p|部分支持/ig)){
              app.post[p].stats.baidu[a17] = "版本：" + a17 + " 及以下【部分支持，可能需要浏览器前缀】"
              css3_s[52] = Number.parseFloat(a17.replace(/[-](\d.*)/ig,".0"));
            }else if(app.post[p].stats.baidu[a17].match(/y|已支持/ig)){
              app.post[p].stats.baidu[a17] = "版本：" + a17 + " 【已支持】"
          ...
          this.setData({
            ['c3Result['+c3temp+']']: {
              ...
              c3UA: app.post[p].usage_perc_a,
              c3UY: app.post[p].usage_perc_y,
              c3Browser: app.post[p].stats,
